...
# 7 mol 7 specs
direct = './7species_7opac_uniform/'
cfile =  './BART_7species_7opac_uniform.cfg'
filename = './7species_7opac_uniform/bestFit.atm'
...
def multi_hist(init_abun, params, species, species_names, filename):
    ...
    plt.savefig('7mol-7specs-Histograms_new.png') 
    plt.savefig('7mol-7specs-Histograms_new.ps')  


# 7 species and their order
species_names = 'CO,CO2,CH4,H2O,NH3,HCN,C2H2'
init_abun = np.array([5e-4, 1e-8, 3e-4, 1e-7, 1e-4, 1e-6, 1e-11]) 
params = [3, 4, 5, 6, 7, 8, 9]
species = [r"${\rm H_{2}O}$", r"${\rm CO_{2}}$", r"${\rm CO}$", r"${\rm CH_{4}}$", r"${\rm NH_{3}}$", r"${\rm HCN}$", r"${\rm C_{2}H_{2}}$"]
